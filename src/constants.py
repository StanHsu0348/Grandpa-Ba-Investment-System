...
# 5年 ROE 趨勢圖的 x 軸標籤、篩選結果表格的展開欄位名稱共用同一份，
# 順序為「舊→新」（對應 reversed(ROE_COLS_RECENT_TO_OLD)），
# 台股／美股頁面都用得到。
...
NET_INCOME_QUICK_OPTIONS = {
    "≥100": 100.0,
    "≥500 (5億)": DEFAULT_NET_INCOME_THRESHOLD,  # 沿用課程門檻常數，避免跟上面的定義各自寫死、之後改一邊漏改另一邊
    "≥1000": 1000.0,
    "不限": 0.0,
}
...
# 交叉比對驗證，信心程度與台股相同（見上方「ROE 欄位時間順序」註解區塊）。
# ---------------------------------------------------------------------------
