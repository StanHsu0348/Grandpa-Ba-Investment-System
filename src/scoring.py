...
def compute_roe_stability(row: pd.Series) -> dict:
    ...
    股票在「個股詳細卡片」與「ROE 模式 E 篩選結果」被分到不同的趨勢
    類別。實測美股 12,497 筆資料裡曾出現 9 筆這種邊界案例（見
    tests/test_scoring_bulk.py）。統一成同一個實作可以從根本消除這個
    不一致，而不是繼續維護兩套「數學等價、數值不等價」的算法。
...
def compute_roe_stability_bulk(df: pd.DataFrame) -> pd.DataFrame:
    ...
    降到毫秒等級。這個函式是 roe_pass_mask() 模式 E（趨勢版）的效能
    瓶頸來源，該模式在側邊欄「符合 N 家」的即時計數裡，不論使用者有
    沒有勾選都會被算一次。
