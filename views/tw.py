...
TW_DETAIL_CARD_SPEC = DetailCardSpec(
    market_cap_col="市值(億)",
    market_cap_display_label="市值(億)",
    ...
    peer_group_col="SECTOR",
    peer_missing_caption="此公司無產業分類資料，無法進行同業比較。",
    peer_header_fn=lambda row, total: f"{row['SECTOR']}（共 {total} 家，依市值排序）",
    sector_check_text_fn=lambda row: f"產業別「{row['SECTOR']}」" if pd.notna(row["SECTOR"]) else "產業別「—」",
    ownership_check_label="董監持股",
    revenue_unit_fn=lambda row: "百萬元",
    peer_extra_column_builder=lambda peers: peers.assign(**{"淨利(億)": peers["預期常利"] / 100.0}),
    peer_extra_show_cols={"淨利(億)": "淨利(億)"},
    peer_extra_column_config={"淨利(億)": st.column_config.NumberColumn(format="%.2f")},
    page_top_anchor="tw-page-top",
)
...
def init_session_defaults():
    defaults = {
        ...
        "_tw_prev_sector_groups": None,  # 用於偵測「新勾選的大分類」，見下方細分產業還原邏輯
...
def clear_all_filters(all_sectors):
    ...
    st.session_state["_tw_prev_sector_groups"] = list(TW_SECTOR_GROUPS.keys())
...
# 側邊欄：資料來源
# ---------------------------------------------------------------------------
with st.sidebar.expander("資料來源 · 台股", expanded=False):
    uploaded_file = st.file_uploader("上傳最新股票清單 Excel", type=["xlsx"], key="tw_uploader")
...
with st.sidebar.expander("① ROE 穩定度篩選", expanded=True):
    ...
    roe_threshold = synced_slider("ROE 門檻（%，0=不限）", "tw_roe_threshold", 0.0, 40.0, 0.5, container=st)
...
with st.sidebar.expander("② 配息率篩選", expanded=False):
    payout_cols = st.columns(len(PAYOUT_QUICK_OPTIONS))
    ...
    payout_threshold = synced_slider("配息率門檻（%，0=不限）", "tw_payout_threshold", 0.0, 100.0, 1.0, container=st)
...
with st.sidebar.expander("④ 淨利門檻（公司夠大）", expanded=False):
    net_income_cols = st.columns(len(NET_INCOME_QUICK_OPTIONS))
    ...
    st.caption("「上市櫃滿 2 年」資料中無此欄位，不做篩選，請自行至公開資訊觀測站查證。")
...
with st.sidebar.expander("③ 產業類別（SECTOR）", expanded=False):
    ...
    init_two_level_sector_state("tw_sector_groups", "tw_sectors", "_tw_prev_sector_groups", tw_group_keys)
    ...
    sync_two_level_sector_state(
        selected_sector_groups, "tw_sectors", "_tw_prev_sector_groups", sectors_in_groups, tw_group_of,
    )
...
st.sidebar.divider()
if st.sidebar.button("重設為不限", key="tw_clear_filters_btn"):
...
# 主畫面：標題與已知限制
# ---------------------------------------------------------------------------
st.markdown('<div id="tw-page-top" tabindex="-1"></div>', unsafe_allow_html=True)
render_hero("台股", data_date, len(df), len(result_df))
...
# 各自佔一個獨立畫面，互不干擾。
# ---------------------------------------------------------------------------
active_view, incoming_symbol = render_research_navigation("tw")

if active_view == "個股研究":
    st.markdown('<div id="tw-research-top" tabindex="-1"></div>', unsafe_allow_html=True)
    if incoming_symbol:
        scroll_to_anchor('tw-research-top')
    ...
    if not tw_search_query.strip():
        render_principles()
    if tw_search_query.strip():
        q = tw_search_query.strip()
        exact_matches = df[df['Symbol'].astype(str).eq(q)]
        linked_query = q == st.session_state.get("_tw_linked_symbol")
        search_matches = exact_matches if not exact_matches.empty or linked_query else search_tw_companies(df, q)
        if len(search_matches) == 0:
            ...
            if search_picked:
                ...
                render_stock_detail(search_row, df, roe_threshold, roe_mode, payout_threshold, key_prefix="tw_search", spec=TW_DETAIL_CARD_SPEC)
...
if active_view == "企業篩選":
    ...
    if len(result_df) == 0:
        ...
        render_download_buttons(table, key_prefix="tw", file_stem="tw_screener_result")
        ...
        if st.session_state.pop("_tw_scroll_to_detail", False):
            ...
            # 這裡捲到詳細檢視區塊，讓「點表格→跳到該股票資訊」真的看得到
            # 「跳過去」的效果。
            scroll_to_anchor("tw-detail-section")
...
if active_view == "IRR 排行":
    ...
    if len(irr_ranking) == 0:
        ...
        st.dataframe(
            ...
            width="stretch",
            hide_index=True,
            key="tw_irr_ranking_select",
            on_select=lambda: select_research_stock(
                "tw", "tw_irr_ranking_select", irr_ranking["Symbol"].tolist()),
            selection_mode="single-row",
            column_config={
                "預期報酬率IRR(%)": st.column_config.NumberColumn(format="%.1f"),
                **{c: st.column_config.NumberColumn(format="%.1f") for c in roe_year_cols},
